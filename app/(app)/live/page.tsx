...
import { CalendarDays, Globe2, MessageCircle, PlayCircle, Radio, Tv } from "lucide-react"
...
export default async function LivePage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div>
      ...
      <section className="grid gap-3 border-b border-border px-4 py-5 sm:grid-cols-3"><div className="rounded-2xl border border-border p-4"><Tv className="size-5 text-brand-red" /><h3 className="mt-3 text-sm font-bold">Africa & Beyond TV</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">A home for programmes and broadcasts. Connect a video provider when a live stream is configured.</p><Link href="/media" className="mt-3 inline-flex text-xs font-semibold text-brand-red">Open Media</Link></div><div className="rounded-2xl border border-border p-4"><PlayCircle className="size-5 text-brand-green" /><h3 className="mt-3 text-sm font-bold">Live broadcasts</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Discover live content shared on the platform without showing an external stream as live when it is not.</p></div><div className="rounded-2xl border border-border p-4"><CalendarDays className="size-5 text-brand-green" /><h3 className="mt-3 text-sm font-bold">Upcoming events</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">A future-ready space for scheduled programmes, conferences and community events.</p></div></section>
      ...
      <section className="px-4 py-5"><div className="flex items-center gap-2"><Globe2 className="size-4 text-brand-green" /><div><h2 className="font-serif text-base font-bold">Live across Africa & Beyond</h2><p className="text-xs text-muted-foreground">Explore live conversations from Africa and the wider world.</p></div></div><Link href="/explore" className="mt-3 inline-flex rounded-full border border-border px-4 py-2 text-xs font-semibold hover:bg-secondary">Explore Africa & Beyond</Link></section>
    </div>
  )
}
