...
export default async function MediaPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div>
      <PageHeader title="MEDIA" subtitle="WIGOD • Photos, video, broadcasts and visual stories" />
      ...
      <section className="border-b border-border px-4 py-4">
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
          {filters.map(({ label, query: filterQuery }) => (
            <Link
              key={label}
              href={filterQuery ? `/media?q=${encodeURIComponent(filterQuery)}` : "/media"}
            ...
            >
              {label === "Photos" ? (
                <ImageIcon className="size-3.5 text-brand-green" />
              ) : label === "Video" ? (
                <Video className="size-3.5 text-brand-green" />
              ) : label === "Live" ? (
                <Radio className="size-3.5 text-brand-green" />
              ) : (
                <Globe2 className="size-3.5 text-brand-green" />
              )}
              {label}
            </Link>
          ))}
      ...
      <section className="border-b border-border bg-gradient-to-br from-brand-green/10 via-background to-brand-red/5 px-4 py-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-serif text-xl font-bold">WIGOD Media</h2>
            <p className="mt-1 max-w-2xl text-sm leading-5 text-muted-foreground">
              Discover photographs, videos, broadcasts and visual moments shared across WIGOD—from Africa to the world.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="border-b border-border px-4 py-5">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h2 className="font-serif text-base font-bold">Featured media</h2>
            <p className="text-xs text-muted-foreground">Visual posts from the WIGOD community</p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {mediaPosts.slice(0, 6).map((post) => (
              <article key={post.id} className="overflow-hidden rounded-2xl border border-border bg-card">
                ...
                <div className="p-3">
                  <p className="line-clamp-3 text-sm leading-5">{post.content}</p>
                  <p className="mt-2 text-xs text-muted-foreground">@{post.author.username}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <section className="px-4 py-5">
        <div className="mb-3">
          <h2 className="font-serif text-base font-bold">Media conversations</h2>
          <p className="text-xs text-muted-foreground">Discuss, repost and react to visual stories</p>
        </div>
        <FeedList
          posts={mediaPosts}
          ...
          empty={
            <EmptyState
              ...
              title="No media conversations yet"
              description="Media posts will appear here when they are shared."
            />
          }
        />
      </section>
