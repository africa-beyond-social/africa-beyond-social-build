    ...
    <div>
      ...
      <section className="border-b border-border px-4 py-5">
        <div className="mb-3 flex items-center justify-between">
          <div><h2 className="font-serif text-base font-bold">Trending now</h2><p className="text-xs text-muted-foreground">Topics people are discussing on the platform</p></div>
        ...
        {trending.length === 0 ? (
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {trending.map(({ tag, count }) => <Link key={tag} href={`/news?q=${encodeURIComponent(tag)}`} className="rounded-xl border border-border p-3 hover:bg-secondary"><p className="text-sm font-semibold">{tag}</p><p className="mt-1 text-xs text-muted-foreground">{count} {count === 1 ? "post" : "posts"}</p></Link>)}
          </div>
        )}
      </section>
      ...
      <section className="px-4 py-5">
        <div className="mb-3">
          <h2 className="font-serif text-base font-bold">{query ? `News conversations for “${query}”` : "Latest news conversations"}</h2>
          <p className="text-xs text-muted-foreground">Posts shared by people across Africa & Beyond</p>
        </div>
