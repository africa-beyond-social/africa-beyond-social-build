  ...
  return (
    <div>
      ...
      <section className="border-b border-border bg-secondary/20 px-4 py-4">
        <div className="mb-2">
          <h2 className="font-serif text-lg font-bold">Search Africa & Beyond</h2>
          <p className="text-xs text-muted-foreground">Find people, posts and conversations from Africa and around the world.</p>
        </div>
        <SearchBar initialQuery={query} />
      </section>

      {query ? <SearchResults query={query} currentUserId={currentUserId} /> : <AfricaExplore currentUserId={currentUserId} />}
    </div>
  )
}
    ...
    <div>
      <section className="border-b border-border bg-gradient-to-br from-brand-green/10 via-background to-brand-red/5 px-4 py-5">
        <div className="mb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm leading-5 text-muted-foreground">Start with Africa, then connect with people, stories and conversations from the wider world.</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {quickLinks.map(({ label, description, icon: Icon, href }) => (
            <Link key={label} href={href} className="group rounded-xl border border-border bg-background/80 p-3 transition-colors hover:bg-secondary">
      ...
      <section id="regions" className="scroll-mt-16 border-b border-border px-4 py-5">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {africaRegions.map((region) => (
            <Link key={region.slug} href={`/explore/regions/${region.slug}`} className="group rounded-xl border border-border p-3 transition-colors hover:bg-secondary">
              <div className="flex items-center justify-between gap-3"><span className="font-semibold">{region.name}</span><ArrowRight className="size-4 text-muted-foreground transition-transform group-hover:translate-x-0.5" /></div>
      ...
      <section id="countries" className="scroll-mt-16 border-b border-border px-4 py-5">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
          {africanCountries.map((country) => <Link key={country.slug} href={`/explore/countries/${country.slug}`} className="rounded-lg border border-border bg-background px-3 py-2 text-sm font-medium transition-colors hover:bg-secondary">{country.name}</Link>)}
        </div>
      </section>
      ...
      <section id="cities" className="scroll-mt-16 border-b border-border px-4 py-5">
        ...
        <div className="flex flex-wrap gap-2">
          {africanCities.map((city) => <Link key={city.slug} href={`/explore/cities/${city.slug}`} className="rounded-full border border-border bg-background px-3 py-1.5 text-sm font-medium hover:bg-secondary">{city.name}</Link>)}
        </div>
      </section>

      <section className="border-b border-border px-4 py-5">
        <div className="mb-3"><h2 className="font-serif text-base font-bold">Explore by interest</h2><p className="text-xs text-muted-foreground">Follow the conversations that matter to you</p></div>
        <div className="grid gap-2 sm:grid-cols-2">
          {exploreLinks.map(({ label, description, icon: Icon, query }) => <Link key={label} href={`/explore?q=${encodeURIComponent(query)}`} className="flex gap-3 rounded-xl border border-border p-3 transition-colors hover:bg-secondary"><div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-accent text-brand-green"><Icon className="size-4" /></div><div className="min-w-0"><p className="font-semibold">{label}</p><p className="mt-0.5 text-xs text-muted-foreground">{description}</p></div><ArrowRight className="ml-auto mt-1 size-4 shrink-0 text-muted-foreground" /></Link>)}
        </div>
      </section>

      <section className="border-b border-border bg-secondary/15 px-4 py-5">
        <div className="mb-3 flex items-start gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-background text-brand-green"><Globe2 className="size-5" /></div><div><h2 className="font-serif text-base font-bold">Beyond Africa</h2><p className="mt-0.5 text-xs leading-5 text-muted-foreground">Africa is our home base, but Africa & Beyond connects the continent to the wider world.</p></div></div>
        <div className="grid gap-2 sm:grid-cols-2">
          {worldRegions.map((region) => <Link key={region.slug} href={`/explore/world/${region.slug}`} className="group rounded-xl border border-border bg-background p-3 transition-colors hover:bg-secondary"><div className="flex items-center justify-between gap-3"><span className="font-semibold">{region.name}</span><ArrowRight className="size-4 text-muted-foreground group-hover:translate-x-0.5" /></div><p className="mt-1 text-xs text-muted-foreground">{region.description}</p></Link>)}
        </div>
      </section>

      <section className="border-b border-border px-4 py-5">
        <div className="mb-3"><h2 className="flex items-center gap-2 font-serif text-base font-bold"><TrendingUp className="size-4 text-brand-red" />Trending across Africa & Beyond</h2><p className="mt-0.5 text-xs text-muted-foreground">What people are talking about right now</p></div>
        {trending.length === 0 ? <div className="rounded-xl border border-dashed border-border px-4 py-6 text-center"><Hash className="mx-auto mb-2 size-5 text-muted-foreground" /><p className="text-sm font-medium">No trends yet</p><p className="mt-1 text-xs text-muted-foreground">Add a #hashtag to a post to start a trend.</p></div> : <div className="grid gap-1 sm:grid-cols-2">{trending.map((t, i) => <Link key={t.tag} href={`/explore?q=${encodeURIComponent(t.tag)}`} className="flex items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-secondary"><span className="flex size-9 items-center justify-center rounded-full bg-accent text-brand-green"><Hash className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{t.tag}</span><span className="text-xs text-muted-foreground">{t.count} {t.count === 1 ? "post" : "posts"}</span></span><span className="text-xs font-medium text-muted-foreground">#{i + 1}</span></Link>)}</div>}
      </section>

      <section className="px-4 py-5">
        <div className="mb-2 flex items-center justify-between"><div><h2 className="font-serif text-base font-bold">Latest across Africa & Beyond</h2><p className="text-xs text-muted-foreground">Recent conversations from the community</p></div><Link href="/explore?q=news" className="text-xs font-semibold text-brand-green hover:underline">Explore more</Link></div>
        <FeedList posts={recent} currentUserId={currentUserId} empty={<EmptyState icon={<Globe2 className="size-6" />} title="Nothing here yet" description="Be the first to post on Africa & Beyond Social." />} />
      </section>
    </div>
  )
}
