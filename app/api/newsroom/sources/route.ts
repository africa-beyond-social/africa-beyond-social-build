...
export async function POST(request: Request) {
  ...
  const focusAreas = Array.isArray(body.focusAreas) ? body.focusAreas.filter((v:any) => typeof v === "string") : []
  ...
  const { data, error } = await createAdminClient().from("news_sources").insert({ name, url, source_type: sourceType, priority, focus_areas: focusAreas, monitoring_enabled: true, created_by: user!.id }).select("*").single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
