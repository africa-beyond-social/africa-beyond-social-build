...
${material}`)
      ...
      const safeForAutoPublish=Number(story.verification_score||0)>=60&&Number(story.independent_source_count||0)>=2&&story.verification_class==="unverified"&&cleanText.includes("Africa &amp; Beyond — News | Analysis | Perspective")&&sourceRows.length>=2
      ...
      const x=(title+" "+published.url).slice(0,280)
      const fb=`${title}\n\n${String(article.dek||story.summary||"").trim()}\n\n${published.url}`.trim()
      const tt=`${title} — ${published.url} #AfricaAndBeyond #News`
      ...
      published++
      await logRun(db,run.id,{story_id:story.id,step:"distribution_ready",message:`Published and prepared social distribution: ${published.url}`,stories_drafted:drafted,articles_ready:articlesReady})
    }
