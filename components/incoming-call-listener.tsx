...
export function IncomingCallListener({currentUserId}:{currentUserId:string}){
 const[call,setCall]=useState<Call|null>(null);const[answered,setAnswered]=useState(false)
 ...
 if(!call)return null
 if(answered)return <VoiceCall session={{...call,status:"accepted"}} caller={false} currentUserId={currentUserId} onClose={()=>{setCall(null);setAnswered(false)}}/>
