  ...
  return (
    <article onClick={() => router.push(postHref)} className={cn("flex cursor-pointer gap-3 border-b border-border px-4 py-3 transition-colors hover:bg-secondary/40", emphasize && "cursor-default hover:bg-transparent")}>
      <div className="flex flex-col items-center">
        <Link href={`/profile/${post.author.username}`} onClick={(e) => e.stopPropagation()} className="shrink-0"><UserAvatar displayName={post.author.display_name} username={post.author.username} avatarUrl={post.author.avatar_url} className="size-10" /></Link>
      </div>
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5 text-sm">
          <Link href={`/profile/${post.author.username}`} onClick={(e) => e.stopPropagation()} className="truncate font-semibold text-foreground hover:underline">{post.author.display_name ?? post.author.username}</Link>
          <span className="truncate text-muted-foreground">@{post.author.username}</span><span className="text-muted-foreground">·</span><time className="shrink-0 text-muted-foreground" dateTime={post.created_at}>{relativeTime(post.created_at)}</time>
          {post.updated_at && post.updated_at !== post.created_at && <span className="shrink-0 text-xs text-muted-foreground">(edited)</span>}
          {isOwner && <div className="ml-auto" onClick={(e) => e.stopPropagation()}><DropdownMenu><DropdownMenuTrigger render={<button aria-label="Post options" className="flex size-8 items-center justify-center rounded-full text-muted-foreground hover:bg-secondary hover:text-foreground"><MoreHorizontal className="size-4" /></button>} /><DropdownMenuContent align="end" className="w-40"><DropdownMenuItem onClick={() => setEditing(true)}><Pencil className="size-4" />Edit</DropdownMenuItem><DropdownMenuItem variant="destructive" onClick={() => setConfirmDelete(true)}><Trash2 className="size-4" />Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>}
        </div>
        ...
        <div className="mt-2 flex max-w-md items-center justify-between text-muted-foreground">
          ...
          <button onClick={onShare} className="group flex items-center gap-1.5 text-sm hover:text-brand-green" aria-label="Share"><span className="flex size-8 items-center justify-center rounded-full group-hover:bg-accent"><Share className="size-[1.15rem]" /></span><span className="sr-only">Share</span></button>
        </div>
      </div>
